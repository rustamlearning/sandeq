    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-gradient-to-r from-[#0A2D52] to-[#1A4A7A] shadow-lg">
        <div className="max-w-3xl mx-auto px-4 py-5 flex items-center gap-3">
          <button onClick={() => router.push('/admin')} className="bg-white/20 hover:bg-white/30 text-white p-2 rounded-lg transition"><ArrowLeft className="w-4 h-4" /></button>
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={kategori} onChange={(e) => setKategori(e.target.value as Kategori)}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-rose-400 outline-none">
                    {KATEGORI.map((k) => <option key={k} value={k}>{KATEGORI_CONFIG[k].icon} {KATEGORI_CONFIG[k].label}</option>)}
                  </select>
                </div>
        ...
        {list.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <button onClick={() => setShowForm(true)} className="mt-4 px-5 py-2 bg-rose-500 text-white rounded-xl text-sm font-semibold hover:bg-rose-600 transition">
              + Buat Pengumuman
            </button>
          </div>
        ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          {p.dipin && <span className="text-xs font-bold text-rose-500">📌 Dipin</span>}
